#!/usr/bin/env bash
# run_noodler.sh
# Runs z3-noodler on ALL files in a single specified folder.
# Results are written to noodler_results/<lib>__<constraints>__<expected>.csv
#
# Usage:  ./run_noodler.sh <lib>/<constraints>/<expected> [base_dir]
#   e.g.  ./run_noodler.sh cJSON/mixed/unsat
#         ./run_noodler.sh b64/string_only/sat /path/to/root
#
...
FOLDER_ARG="${1:-}"
BASE_DIR="${2:-.}"
OUT_DIR="noodler_results"
SOLVER="$(dirname "$0")/z3-noodler/build/z3"
...
if ! command -v "$SOLVER" >/dev/null 2>&1; then
    echo "Error: '$SOLVER' not found in PATH." >&2
    echo "Set the SOLVER variable at the top of this script to the full path if needed." >&2
    exit 1
fi
...
run_one() {
    ...
    t0=$(ms_now)
    # z3-noodler accepts SMT-LIB2 directly; smt.string_solver=noodler activates the string solver
    # Capture stdout only; discard stderr (z3-noodler emits harmless parameter warnings to stderr)
    output=$(timeout "$TIMEOUT" "$SOLVER" smt.string_solver=noodler "$smt_file" 2>/dev/null)
    exit_code=$?
    t1=$(ms_now)
    ELAPSED_MS=$(( t1 - t0 ))
    ...
    if [ $exit_code -eq 124 ]; then
        ...
        # z3-noodler may exit 1 due to unknown SMT options (e.g. :incremental) but still
        # prints sat/unsat — so ignore exit code and parse output directly
