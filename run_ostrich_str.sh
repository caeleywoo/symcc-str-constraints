...
TIMEOUT=60
OUTPUT=~/benchmark_constraints/ostrich_str_results.csv
PROGRAMS="cJSON inih b64 minicsv yuarel"

echo "program,file,type,result,time_ms" > $OUTPUT

total=$(find ~/benchmark_constraints/string_only_constraints -name "*.smt2" | wc -l)
count=0

for program in $PROGRAMS; do
    for type in sat unsat; do
        dir=~/benchmark_constraints/string_only_constraints/$program/$type
